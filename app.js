...
// Build embedded YouTube player (no new tab, plays inside app)
function buildVideoPlayer(youtubeUrl, label) {
  const videoId = getYouTubeId(youtubeUrl);
  if (!videoId) return '';
  return `
    <div class="video-player-wrap">
      <div class="video-player-label">${label || 'TUTORIAL VIDEO'}</div>
...
function openExModal(key) {
  ...
  const videoSection = buildVideoPlayer(ex.youtube, 'HOW TO DO IT — TUTORIAL');
...
function closeExModal() {
  // Stop video playback when closing modal by clearing iframe src
  const iframe = document.querySelector('#ex-modal iframe');
...
function setKegelLevel(level) {
  ...
  prog.exercises.forEach((ex, idx) => {
    // Build embedded video player for this exercise
    const videoHtml = ex.youtube ? `
      <div class="kegel-video-wrap">
        <div class="kegel-video-label">▶ TUTORIAL VIDEO</div>
        <div class="video-player-container">
          <iframe
            src="${ex.youtube}"
            title="${ex.name} kegel exercise tutorial"
            frameborder="0"
            allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
            allowfullscreen
            class="video-player-iframe"
      ...
      <div class="kegel-how-to">
        ...
        <div class="kegel-anatomy-visual">
          <div class="anatomy-diagram">
            <svg viewBox="0 0 120 80" width="100" height="68">
